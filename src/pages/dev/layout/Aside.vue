  ...
  <aside class="flex flex-col">
    <el-menu class="menu h-full" router :collapse="isCollapse" :default-active="$route.path">
      <el-tooltip v-for="item in menus" :key="item.to" :content="item.name" :disabled="!isCollapse" placement="right">
        <el-menu-item :index="item.to" :route="item.to">
          <svg-icon :inline="icons[item.icon]" class="menu-svg-icon text-h6" :class="{ 'text-h5': isCollapse }" />
          <span class="ml-2" :class="{ invisible: isCollapse }">{{ item.name }}</span>
        </el-menu-item>
      </el-tooltip>
    </el-menu>
    <div class="p-2 text-center hover:bg-gray-100 cursor-pointer border-t border-t-gray-200" @click="isCollapse = !isCollapse">
